...
class PHPCrawler
{
     ...
     */
    protected PHPCrawlerHTTPRequest $PageRequest;

    /**
     * The PHPCrawlerLinkCache-Object
     *
     * @var PHPCrawlerSQLiteURLCache|PHPCrawlerMemoryURLCache
     */
    protected $LinkCache;
     ...
     *
     * @var PHPCrawlerSQLiteCookieCache|PHPCrawlerMemoryCookieCache
     */
     ...
     */
    protected PHPCrawlerURLFilter $UrlFilter;
     ...
     */
    protected PHPCrawlerRobotsTxtParser $RobotsTxtParser;
     ...
     */
    protected PHPCrawlerUserSendDataCache $UserSendDataCache;
     ...
     * @val bool
     */
    protected bool $obey_robots_txt = false;
     ...
     * @var string
     */
    protected string $robots_txt_uri;
     ...
     * @var bool
     */
    protected bool $only_count_received_documents = true;
     ...
     * @var bool
     */
    protected bool $cookie_handling_enabled = true;
     ...
     */
    protected int $url_cache_type = 1;
     ...
     * @var string
     */
    protected string $working_base_directory;
    ...
    protected array $link_priority_array = [];
     ...
     * @var int
     */
    protected int $child_process_number;

    protected int $child_process_count = 1;
     ...
     */
    protected PHPCrawlerProcessHandler $ProcessHandler;
     ...
     */
    protected PHPCrawlerStatusHandler $CrawlerStatusHandler;
     ...
     */
    protected int $multiprocess_mode = 0;
     ...
     */
    protected PHPCrawlerDocumentInfoQueue $DocumentInfoQueue;

    protected bool $follow_redirects_till_content = true;
     ...
     * @throws Exception
     */
    public function goMultiProcessed(int $process_count = 3, int $multiprocess_mode = 1): void
    {
        $this->multiprocess_mode = $multiprocess_mode;
        $this->child_process_count = $process_count;

        // @todo switch OS and handle multiprocess differently
        switch (PHPCrawlerUtils::getOS()) {

            // Windows
            case PHPCrawlerUtils::OS_NIX:
                $this->goMultiProcessedLinux();
                break;

            //Linux
            case PHPCrawlerUtils::OS_WINDOWS:
                $this->goMultiProcessedWindows();
                break;
        }
     ...
     * @throws Exception
     */
    public function goMultiProcessedLinux(int $process_count = 3, int $multiprocess_mode = 1): void
    {
     ...
     * @throws Exception
     */
    public function goMultiProcessedWindows(int $process_count = 3, int $multiprocess_mode = 1): void
    {
     ...
     */
    protected function startChildProcessLoop(): void
    {
        $this->initCrawlerProcess();
    ...
    protected function processUrl(PHPCrawlerURLDescriptor $UrlDescriptor): bool
    {
        ...
        PHPCrawlerBenchmark::resetAll(['crawling_process']);

        return false;
    }

    protected function processRobotsTxt(): void
    {
     ...
     */
    protected function setupCrawlerStatusHandler(): void
    {
     ...
     * @section 1 Basic settings
     */
    public function getProcessReport(): PHPCrawlerProcessReport
    {
     ...
     */
    public function handleHeaderInfo(PHPCrawlerResponseHeader $header): int
    {
        return 1;
    }
     ...
     * @section 3 Overridable methods / User data-processing
     */
    public function initChildProcess(): void
    {
    }
     ...
     */
    public function addLinkPriority(string $regex, int $level): ?bool
    {
     ...
     * @section 10 Other settings
     */
    public function setFollowRedirectsTillContent(bool $mode): void
    {
    ...
    {
        // Check mode
        if (!preg_match('/^[0-3]{1}$/', $follow_mode)) {
            return false;
        }
     ...
     */
    public function addContentTypeReceiveRule(string $regex): bool
    {
     ...
     * @section 2 Filter-settings
     */
    public function addURLFollowRule(string $regex): bool
    {
     ...
     * @section 2 Filter-settings
     */
    public function addURLFilterRule(string $regex): bool
    {
     ...
     * @section 10 Other settings
     */
    public function addStreamToFileContentType(string $regex): bool
    {
     ...
     * @param string|null $robots_txt_uri Optionally. The URL or path to the robots.txt-file to obey as URI (like "http://mysite.com/path/myrobots.txt"
     ...
     * @section 2 Filter-settings
     */
    public function obeyRobotsTxt(bool $mode, string $robots_txt_uri = null): bool
    {
        if (!is_bool($mode)) {
            return false;
        }
     ...
     */
    public function setRequestLimit(int $limit, bool $only_count_received_documents): bool
    {
     ...
     * @section 10 Other settings
     */
    public function enableCookieHandling(bool $mode): bool
    {
        if (!is_bool($mode)) {
            return false;
        }
     ...
     */
    public function enableAggressiveLinkSearch(bool $mode): bool
    {
     ...
     *
     * Example:
     * <code>$crawler->setLinkExtractionTags(["href", "src"]);</code>
     ...
     * @return bool
     */
    public function setLinkExtractionTags(array $tag_array): bool
    {
     ...
     * @section 10 Other settings
     */
    public function addBasicAuthentication(string $url_regex, string $username, string $password): bool
    {
     ...
     * @param string|null $proxy_username Optional. The username for proxy-authentication or NULL if no authentication is required.
     * @param string|null $proxy_password Optional. The password for proxy-authentication or NULL if no authentication is required.
     *
     * @section 10 Other settings
     */
    public function setProxy(string $proxy_host, int $proxy_port, string $proxy_username = null, string $proxy_password = null): void
    {
     ...
     * @section 6 Linkfinding settings
     */
    public function addLinkSearchContentType(string $regex): bool
    {
     ...
     * @section 1 Basic settings
     */
    public function setUrlCacheType(int $url_cache_type): ?bool
    {
     ...
     * @section 2 Filter-settings
     */
    public function obeyNoFollowTags(bool $mode): void
    {
     ...
     * Example
     * <code>
     * $post_data = ["username" => "me", "password" => "my_password", "action" => "do_login");
     ...
     * @section 10 Other settings
     */
    public function addPostData(string $url_regex, array $post_data_array): bool
    {
     ...
     */
    public function getCrawlerId(): int
    {
     ...
     * @throws Exception
     */
    public function resume(int $crawler_id): void
    {
        ...
        if (file_exists($this->working_directory . 'pids')) {
            unlink($this->working_directory . "pids");
        }
    }
     ...
     * @section 9 Process resumption
     */
    public function enableResumption(): void
    {
     ...
     * @section 1 Basic settings
     */
    public function setHTTPProtocolVersion(int $http_protocol_version): bool
    {
     ...
     * @section 10 Other settings
     */
    public function requestGzipContent(bool $mode): void
    {
        $this->PageRequest->requestGzipContent($mode);
    }
     ...
     * @section 6 Linkfinding settings
     */
    public function excludeLinkSearchDocumentSections(int $document_sections): void
    {
        $this->PageRequest->excludeLinkSearchDocumentSections($document_sections);
    }
     ...
     * @return bool
     */
    public function setCrawlingDepthLimit(int $depth): bool
    {
