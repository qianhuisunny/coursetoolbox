...
class ProductListShortcode extends Shortcode {
    ...
    public function render(): string {
        ...
        $product_list_type = ! empty( $atts['product_list_type'] ) ? sanitize_text_field( $atts['product_list_type'] ) : '';
        $keywords          = ! empty( $atts['keywords'] ) ? sanitize_text_field( $atts['keywords'] ) : '';

        if ( ! in_array( $product_list_type, AmplitudeActions::AFFILIATE_LIST_ALLOWED_PRODUCT_TYPES, true ) ) {
            return __( 'Wrong product list type selected.', 'hostinger-affiliate-plugin' );
        }

        if ( $product_list_type === AmplitudeActions::AFFILIATE_LIST_BESTSELLERS_PRODUCT_TYPE ) {
            if ( empty( $keywords ) ) {
                return __( 'Please enter keywords!', 'hostinger-affiliate-plugin' );
            }
            ...
            try {
                $products = $this->product_fetch_service->pull_products( $asin, $atts['display_type'] );
            } catch ( \Exception $e ) {
                return $e->getMessage();
            }
        }
