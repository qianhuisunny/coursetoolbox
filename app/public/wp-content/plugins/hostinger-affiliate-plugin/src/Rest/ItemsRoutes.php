...
class ItemsRoutes {
    ...
    public function __construct( PluginSettings $plugin_settings, ProductRepository $product_repository, ProductFetchService $product_fetch_service ) {
    ...
    public function search_items( WP_REST_Request $request ): WP_REST_Response|AmazonApiError|WP_Error {
        ...
        $response = $this->product_fetch_service->search_items( $search_keyword );
    ...
    public function validate_items( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        try {
            $products = $this->product_fetch_service->pull_products( $items, $layout );
        } catch ( \Exception $e ) {
            $errors = array(
