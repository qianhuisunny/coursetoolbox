 ...
 * @LastEditors: iowen
 * @LastEditTime: 2024-04-10 18:19:12
...
function add_customize_scripts(){
    wp_register_style('admin-options', CSF::include_plugin_url('customize/css/options.css'), array(), IO_VERSION, '');
    wp_enqueue_style('admin-options'); 
    
    wp_register_script( 'admin-options', CSF::include_plugin_url('customize/js/options.js'), array('jquery'), IO_VERSION, true );
...
function io_get_system_info(){
    global $wpdb;
    $_theme      = wp_get_theme();
    $_theme_name = sprintf('%1$s (%2$s)', $_theme->name, $_theme->stylesheet);
    ...
    if (method_exists('WP_Object_Cache', 'get_mc')) {
        ...
        $memcached = '<div class="ajax-form" style="margin-top:20px;"><h4><strong>Memcached</strong>： 已启用 <a class="button button-primary ajax-get" style="vertical-align:inherit" href="' . $r_cache . '">刷新缓存</a></h4>';
        $memcached .= '<div class="ajax-notice"></div>';
        $memcached .= '<ul style="margin-left:10px">';
    ...
    return $system . $memcached . $opcache;
}
