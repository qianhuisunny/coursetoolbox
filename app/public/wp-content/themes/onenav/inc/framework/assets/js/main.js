        ...
        if ( value.length > 1 ) { // iowen.cn 3 to 1 兼容中文
          ...
          $titles.each( function() {
            ...
            if ( $title.text().match( new RegExp('.*?' + value + '.*?', 'i') ) ) {
              ...
              /* iowen 添加父级目录 */
              var tab = $field.parent().data('section-id');
              var url = window.location.href.split('#tab')[0] + '#tab=' + tab;
              var $html = "<p class='search-list-name'><b>目录</b><a href='" + url + "' target='_blank'>" + decodeURIComponent(tab) + " <i class='fa fa-angle-right' aria-hidden='true'></i></a></p>";
              if(!$field.find('.search-list-name')[0])
                $field.prepend($html);
              /* iowen END */
            }
          ...
          $('.search-list-name').remove(); // iowen 删除父级目录
  ...
  $.fn.csf_field_group = function() {
    return this.each( function() {
      ...
      $group.children('.csf-cloneable-add').on('click', function( e ) {
        ...
        if (_index[0] && count > 0) {
          let inputs = $wrapper.find('.csf-cloneable-item input[name*="index"]');
          let maxVal = -Infinity;  // 初始化最大值为负无穷大
          for (let i = 0; i < inputs.length; i++) {
            let val = Number(inputs[i].value);  // 获取当前 input 的值并转为数字类型
            if (!isNaN(val)) {  // 确保值是数字类型
              maxVal = Math.max(maxVal, val);  // 找到最大值
            }
          }
          _index.val(maxVal+1);
        }
