 ...
 * @LastEditors: iowen
 * @LastEditTime: 2023-04-04 00:14:15
...
function io_set_user_bookmark_rewrite_rules($wp_rewrite)
{
    if (get_option('permalink_structure')) {
        ...
        $lang = io_get_lang_rules(); 
        if($lang){
            $new_rules[$lang . 'bookmark/([^/]+)$'] = 'index.php?lang=$matches[1]&bookmark_id=$matches[2]';
            $new_rules[$lang . 'bookmark/?$']       = 'index.php?lang=$matches[1]&bookmark_id=default';
        }
...
function io_handle_user_child_routes_rewrite($wp_rewrite)
{
    if (get_option('permalink_structure')) {
        ...
        // 分页
        $new_rules['user/([a-zA-Z]+)/page/?([0-9]{1,})/?$'] = 'index.php?user_child_route=$matches[1]&is_user_route=1&page=$matches[2]';
        $new_rules['user/([a-zA-Z]+)/([a-zA-Z]+)/page/?([0-9]{1,})/?$'] = 'index.php?user_child_route=$matches[1]&user_grandchild_route=$matches[2]&is_user_route=1&page=$matches[3]';

        $lang = io_get_lang_rules(); 
        if($lang){
            $new_rules[$lang . 'user/([a-zA-Z]+)$']             = 'index.php?lang=$matches[1]&user_child_route=$matches[2]&is_user_route=1';
            $new_rules[$lang . 'user/([a-zA-Z]+)/([a-zA-Z]+)$'] = 'index.php?lang=$matches[1]&user_child_route=$matches[2]&user_grandchild_route=$matches[3]&is_user_route=1';
            // 分页
            $new_rules[$lang . 'user/([a-zA-Z]+)/page/?([0-9]{1,})/?$']             = 'index.php?lang=$matches[1]&user_child_route=$matches[2]&is_user_route=1&page=$matches[3]';
            $new_rules[$lang . 'user/([a-zA-Z]+)/([a-zA-Z]+)/page/?([0-9]{1,})/?$'] = 'index.php?lang=$matches[1]&user_child_route=$matches[2]&user_grandchild_route=$matches[3]&is_user_route=1&page=$matches[4]';
        }
 ...
 * --------------------------------------------------------------------------------------
 */
function io_resize_img($ori, $dst = '', $dst_width = 100, $dst_height = 100, $delete_ori = false){ //绝对路径, 带文件名
...
function getNotificationData($user_id = 0, $type = 'all', $page = 1, $limit = 20) {
    ...
    $pagination_base = home_url() . (is_array($type) ? '/user/notifications/all/page/%#%' : '/user/notifications/' . $type . '/page/%#%');
    return (object)array(
        'count' => $count,
