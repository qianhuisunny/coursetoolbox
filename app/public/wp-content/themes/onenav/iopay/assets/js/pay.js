...
function pay_ajax(_data,_this) {
    ...
    $.ajax({
        ...
        if (n.url_qrcode) {
            ...
            var tips = localize.scanQRPay;
            if (n.expiration) {
                tips = n.expiration;
            }
            var html = '<div class="pay-qr '+n.pay_method+'">\
                <div class="pay-header my-4"><span class="pay-logo"></span><span class="pay-name title-wechat">'+localize.weChatPay+'</span><span class="pay-name title-alipay">'+localize.alipay+'</span></div>\
