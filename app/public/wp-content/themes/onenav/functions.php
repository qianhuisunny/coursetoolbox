...
function onenav_enqueue_styles() {
    // Enqueue the main stylesheet (style.css) from the root of the theme directory
    wp_enqueue_style('onenav-style', get_stylesheet_uri());

    // If you have a specific CSS file like css/style.css
    // wp_enqueue_style('onenav-custom-style', get_template_directory_uri() . '/css/style.css');
}
add_action('wp_enqueue_scripts', 'onenav_enqueue_styles');
